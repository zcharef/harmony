...
async function openServerMenuAndCreateInvite(page: Page): Promise<string> {
	...
	const responsePromise = page.waitForResponse(
		(response) =>
			response
				.url()
				.includes(`/v1/servers/${HARMONY_DEV_SERVER_ID}/invites`) &&
...
test.describe("Invites", () => {
	test("should create an invite and display the code", async ({ page }) => {
		...
		await expect(
			page.locator('[data-test="create-invite-dialog"]'),
		).not.toBeVisible();
	});

	test("should preview an invite code in the join dialog", async ({
		page,
	}) => {
		await loginAsAlice(page);
		await navigateToHarmonyDev(page);
		...
		const memberCount = page.locator(
			'[data-test="join-server-member-count"]',
		);
