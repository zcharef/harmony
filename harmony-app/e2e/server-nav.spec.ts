...
test.describe("Server Navigation", () => {
	...
	test("should display server list with at least one server", async ({
		page,
	}) => {
	...
	test("should show channel sidebar when clicking a server", async ({
		page,
	}) => {
		await page
			.locator(
				'[data-test="server-button"][data-server-id="cccccccc-cccc-cccc-cccc-cccccccccccc"]',
			)
			.click();
