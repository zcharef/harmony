...
test.describe('DM Lifecycle', () => {
  ...
  test.describe('sidebar ordering', () => {
    test('most recently active DM appears first in sidebar', async ({ page }) => {
      ...
      await sendMessage(orderUser.token, dmA.channelId, `Msg to A ${Date.now()}`)

      // WHY: Small delay ensures the timestamps are distinct so ordering is deterministic.
      ...
      await sendMessage(orderUser.token, dmB.channelId, `Msg to B ${Date.now()}`)
