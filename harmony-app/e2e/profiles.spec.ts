...
test.describe('Profile & Username Display', () => {
  ...
  test('member username includes the user-factory prefix', async ({ page }) => {
    ...
    // WHY: The username starts with "prof-member" or "prof_member" — the backend may
    // normalize hyphens to underscores in display_name. Match either separator.
    await expect(username).toContainText(/prof[-_]member/, { timeout: 15_000 })
  })
  ...
  test('new member appears in member list after joining server', async ({ page }) => {
    ...
    // WHY: The member list query must resolve and include the newly joined member.
    // In CI, API + query resolution can take longer than the default expect timeout.
    ...
    await expect(username).toContainText(/prof[-_]newjoin/, { timeout: 15_000 })
  })
})
