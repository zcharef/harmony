...
test.describe('Error Recovery', () => {
  ...
  test.describe('banned user API errors', () => {
    test('banned user receives 403 when attempting to rejoin', async () => {
      ...
      const rejoinRes = await joinServerRaw(banTarget.token, banServer.id, invite2.code)
  ...
  test.describe('stale data handling', () => {
    test('user can navigate to DM home after being kicked from a server', async ({ page }) => {
      ...
      // After kick, the app may briefly show an error or loading state for the
      // stale server. Wait longer for the server-list (where dm-home-button lives)
      // to fully render before clicking.
      const dmHomeButton = page.locator('[data-test="dm-home-button"]')
      await dmHomeButton.waitFor({ state: 'visible', timeout: 20_000 })
      await dmHomeButton.click()

      // WHY: After clicking DM home, wait for the DOM to settle before asserting.
      // networkidle is unreliable with persistent WebSocket connections, so we use
      // domcontentloaded instead and let the dm-sidebar assertion handle the real wait.
      await page.waitForLoadState('domcontentloaded')

      // WHY: DM sidebar should render without errors. The view switch from
      // server→dms involves state updates + re-render. Give CI enough time.
      const dmSidebar = page.locator('[data-test="dm-sidebar"]')
      await expect(dmSidebar).toBeVisible({ timeout: 20_000 })
    })
  })
})
