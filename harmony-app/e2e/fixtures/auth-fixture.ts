...
export async function authenticatePage(
  ...
  // SSE now connects independently via Bearer token (no syncProfile gate),
  // so we only need to wait for the layout to render.
...
export async function selectChannel(page: Page, channelName: string): Promise<void> {
  const channelButton = page.locator(
    `[data-test="channel-button"][data-channel-name="${channelName}"]`,
  )
