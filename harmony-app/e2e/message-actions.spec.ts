...
async function navigateToGeneralChannel(page: Page) {
	await page
		.locator(
			`[data-test="server-button"][data-server-id="${HARMONY_DEV_SERVER_ID}"]`,
		)
		.click();
	await page
		.locator('[data-test="channel-sidebar"]')
		.waitFor({ timeout: 10000 });
	await page
		.locator(
			`[data-test="channel-button"][data-channel-id="${GENERAL_CHANNEL_ID}"]`,
		)
		.click();
	await page
		.locator('[data-test="chat-area"]')
		.waitFor({ timeout: 10000 });
	await page
		.locator('[data-test="message-item"]')
		.first()
		.waitFor({ timeout: 10000 });
}

test.describe("Message Actions", () => {
	test("should edit own message and verify content changes", async ({
		page,
	}) => {
		...
		const editButton = targetMessage.locator(
		...
		const editInput = editingMessage.locator(
		...
		await expect(editInput).toHaveValue(
			"Welcome to Harmony! (edited via E2E)",
		);
		...
		const responsePromise = page.waitForResponse(
			(response) =>
				response
					.url()
					.includes(
						`/v1/channels/${GENERAL_CHANNEL_ID}/messages/${messageId}`,
					) && response.request().method() === "PATCH",
		);

		await editingMessage
			.locator('[data-test="message-edit-save"]')
			.click();
	...
	test("should delete own message and verify it disappears", async ({
		...
		const sendResponsePromise = page.waitForResponse(
			(response) =>
				response
					.url()
					.includes(
						`/v1/channels/${GENERAL_CHANNEL_ID}/messages`,
		...
		const newMessage = page.locator(
			`[data-test="message-item"][data-message-id="${newMessageId}"]`,
		);
		...
		const deleteButton = newMessage.locator(
		...
		const deleteResponsePromise = page.waitForResponse(
			(response) =>
				response
					.url()
					.includes(
						`/v1/channels/${GENERAL_CHANNEL_ID}/messages/${newMessageId}`,
