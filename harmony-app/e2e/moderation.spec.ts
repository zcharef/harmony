...
test.describe('Kick & Ban Moderation', () => {
  ...
  // WHY: Shared invite reused across all tests. The free plan limits active invites
  // to 5 — creating per-test invites exhausts this quickly and causes 403 errors.
  let sharedInvite: { code: string }
  ...
  test.beforeAll(async () => {
    ...
    sharedInvite = await createInvite(owner.token, server.id)
    for (const u of [admin, mod, member]) await joinServer(u.token, server.id, sharedInvite.code)
  ...
  test('moderator kicks member via context menu', async ({ page }) => {
    ...
    await joinServer(kickTarget.token, server.id, sharedInvite.code)
  ...
  test('moderator cannot kick fellow moderator — same rank blocked', async ({ page }) => {
    ...
    await joinServer(mod2.token, server.id, sharedInvite.code)
  ...
  test('admin bans member via context menu with reason', async ({ page }) => {
    ...
    await joinServer(banTarget.token, server.id, sharedInvite.code)
  ...
  test('admin unbans user via settings bans tab', async ({ page }) => {
    ...
    await joinServer(unbanTarget.token, server.id, sharedInvite.code)
  ...
  test('ban list visible to admin', async ({ page }) => {
    ...
    await joinServer(banListTarget.token, server.id, sharedInvite.code)
    ...
    // WHY: Wait for the specific ban row, not just the container div.
  ...
  test('kicked user can rejoin via invite', async ({ page }) => {
    ...
    await joinServer(rejoinTarget.token, server.id, sharedInvite.code)

    // Kick via API
    await kickMember(admin.token, server.id, rejoinTarget.id)

    // Rejoin using the shared invite (kick does NOT invalidate invites)
    await joinServer(rejoinTarget.token, server.id, sharedInvite.code)
  ...
  test('banned user cannot rejoin via invite', async ({ page }) => {
    ...
    await joinServer(bannedTarget.token, server.id, sharedInvite.code)

    // Ban via API
    await banUser(admin.token, server.id, bannedTarget.id, 'banned for test')

    // Attempt to rejoin — should fail (ban prevents it regardless of valid invite)
    const rejoinRes = await joinServerRaw(bannedTarget.token, server.id, sharedInvite.code)
    ...
    await expect(bannedItem).not.toBeVisible({ timeout: 5_000 })
