...
/**
 * Global profile settings modal (display name, custom status, avatar).
 * Opened via the gear button in both sidebars; mounted once in MainLayout.
 */
export function ProfileSettingsModal() {
  const isOpen = useSettingsUiStore((s) => s.showProfileSettings)
  const closeProfileSettings = useSettingsUiStore((s) => s.closeProfileSettings)
  const { data: profile, isPending } = useCurrentProfile()

  // WHY unmount when closed: guarantees the form re-reads fresh defaults from
  // the profile cache on every open (load-then-render, CLAUDE.md 4.4 — no
  // useEffect reset).
  if (isOpen === false) return null

  return (
    <Modal isOpen onClose={closeProfileSettings} size="md" data-test="profile-settings-modal">
      <ModalContent>
        {isPending || profile === undefined ? (
          <ModalBody className="items-center py-10">
            <Spinner size="sm" />
          </ModalBody>
        ) : (
          <ProfileSettingsForm profile={profile} onClose={closeProfileSettings} />
        )}
      </ModalContent>
    </Modal>
  )
...
function ProfileSettingsForm({
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)}>
      <ModalHeader>{t('profileSettingsTitle')}</ModalHeader>
      <ModalBody>
